...
void init_cmd() {
    ...
    err_cnt = 0;
    quit_flag = false;
    add_cmd("help", do_help_cmd,       "                | Mostra documentacao");
    add_cmd("option", do_option_cmd,   " [name val]     | Mostra ou configura opcoes");
    add_cmd("quit", do_quit_cmd,       "                | Sai do programa");
    add_cmd("source", do_source_cmd,
	    " file           | Le comandos de um arquivo fonte");
    add_cmd("log", do_log_cmd,         " file           | Copia a saida para o arquivo");
    add_cmd("time", do_time_cmd,       " cmd arg ...    | Tempo de execucao do comando");
    add_cmd("#", do_comment_cmd,       " ...            | Mostra comentario");
    add_param("verbose", &verblevel, "Nivel de relatorio", NULL);
    add_param("error", &err_limit,   "Numero de erros antes de sair", NULL);
    add_param("echo", &echo, "Mostra/Nao comandos", NULL);
#if 0
    add_param("megabytes", &mblimit, "Tamanho maximo em megabytes aceitavel", NULL);
    add_param("segundos", &timelimit, "Tempo maximo aceitavel", change_timeout);
#endif
    init_in();
    init_time(&last_time);
...
void record_error() {
	...
	report(1, "Erro de limite execedido. Parando a execucao do comando");
	quit_flag = true;
    }
}
...
static bool interpret_cmda(int argc, char *argv[]) {
	...
	report(1, "Comando desconhecido '%s'", argv[0]);
	record_error();
	ok = false;
    }
    return ok;
...
bool interpret_cmd(char *cmdline) {
	...
	return false;
#if RPT >= 6
    report(6, "Interpretando commando '%s'\n", cmdline);
#endif
...
void add_quit_helper(cmd_function qf) {
	...
	report_event(MSG_FATAL, "Limite execedido na ajuda de saida");
}
...
bool do_option_cmd(int argc, char *argv[]) {
	...
	if (i+1 >= argc) {
	    report(1, "Nenhum valor dado para o parametro %s", name);
	    return false;
	} else if (!get_int(argv[++i], &value)) {
	    report(1, "Nao foi possivel traduzir '%s' como inteiro", argv[i]);
	    return false;
	}
	...
	if (!found) {
	    report(1, "Parametro desconhecido '%s'", name);
	    return false;
	}
    }
    return true;
...
bool do_source_cmd(int argc, char *argv[]) {
    if (argc < 2) {
	report(1, "Nenhum arquivo fonte dado");
	return false;
    }
    if (!push_file(argv[1])) {
	report(1, "Nao foi possivel abrir o arquivo fonte '%s'", argv[1]);
	return false;
    }
    return true;
}
...
bool do_log_cmd(int argc, char *argv[]) {
    if (argc < 2) {
	report(1, "Nenhum arquivo de log dado");
	return false;
    }
    bool result = set_logfile(argv[1]);
    if (!result) {
	report(1, "Nao foi possivel abrir o arquivo de log '%s'", argv[1]);
    }
    return result;
}
...
bool do_time_cmd(int argc, char *argv[]) {
	...
	report(1, "Tempo passado = %.3f, Diferenca de tempo = %.3f", elapsed, delta);
    } else {
	...
	if (block_flag) {
	    ...
	    report(1, "Diferenca de tempo = %.3f", delta);
	}
    }
    return ok;
}
...
bool start_cmd(char *infile_name) {
    ...
    if (!ok)
	report(1, "Nao foi possil abrir o arquivo fonte '%s'",
	       infile_name ? infile_name : "entrada padrao");
    return ok;
}
...
bool run_console(char *infile_name) {
	...
	report(1, "ERROR: Nao foi possivel abrir o arquivo fonte '%s'", infile_name);
	return false;
    }
    while (!cmd_done()) {
